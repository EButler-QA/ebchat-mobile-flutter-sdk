...
/// Different types of hints that can be shown in [CustomStreamMessageInput].
enum HintType {
  /// Hint for [CustomStreamMessageInput] when the command is enabled and the command
  /// is 'giphy'.
  searchGif,

  /// Hint for [CustomStreamMessageInput] when there are attachments.
  addACommentOrSend,

  /// Hint for [CustomStreamMessageInput] when slow mode is enabled.
  slowModeOn,

  /// Hint for [CustomStreamMessageInput] when other conditions are not met.
  writeAMessage,
}

/// Function that returns the hint text for [CustomStreamMessageInput] based on
/// [type].
...
class CustomStreamMessageInput extends StatefulWidget {
  ...
  /// Defines if the [CustomStreamMessageInput] loses focuses after a message is sent.
  ...
  /// Wrap [CustomStreamMessageInput] with a [SafeArea widget]
  final bool? enableSafeArea;

  /// Elevation of the [CustomStreamMessageInput]
  final double? elevation;

  /// Shadow for the [CustomStreamMessageInput] widget
  ...
  @override
  CustomStreamMessageInputState createState() =>
      CustomStreamMessageInputState();
}

/// State of [CustomStreamMessageInput]
class CustomStreamMessageInputState extends State<CustomStreamMessageInput>
    with RestorationMixin<CustomStreamMessageInput>, WidgetsBindingObserver {
  ...
  void _createLocalController([Message? message]) {
    ...
    _controller = StreamRestorableMessageInputController();
  }
  ...
  @override
  void didUpdateWidget(covariant CustomStreamMessageInput oldWidget) {
      ...
      builder: (context, value, _) {
        Widget child = DecoratedBox(
          decoration: BoxDecoration(
            color: widget.globalThreadReply
                ? const Color(0xFFffe585)
                  ...
                  LimitedBox(
                    ...
                    child: PlatformWidgetBuilder(
                      ...
                      child: StreamMessageTextField(
                        ...
                        onChanged: (!widget.threadReply)
                            ? (_) {
                                channel.keyStroke();
                              }
                            : null,
                        onEditingComplete: (!widget.threadReply)
                            ? () {
                                channel.stopTyping();
                              }
                            : null,
  ...
  void _checkContainsUrl(String value, BuildContext context) async {
    ...
    final matchedUrls = _urlRegex.allMatches(value).where((it) {
      ...
      return parsedMatch.host.split('.').last.isValidTLD() &&
          widget.ogPreviewFilter.call(parsedMatch, value);
    }).toList();
  ...
  Future<void> sendMessage() async {
    ...
    if (!channel.ownCapabilities.contains(PermissionType.sendLinks) &&
        _urlRegex.allMatches(message.text ?? '').any(
              (element) =>
