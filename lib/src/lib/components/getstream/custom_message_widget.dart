...
/// [CustomStreamMessageListView].
///
/// The widget components render the ui based on the first ancestor of type
/// [StreamChatTheme].
/// Modify it to change the widget appearance.
/// {@endtemplate}
class CustomStreamMessageWidget extends StatefulWidget {
  /// Creates a new instance of the message widget.
  CustomStreamMessageWidget({
    super.key,
          ...
          'image': (context, message, attachments) {
            ...
            if (attachments.length > 1) {
              return Wrap(
                ...
                children: attachments
                    .map(
                      (attachment) => wrapAttachmentWidget(
                        context,
            ...
            return wrapAttachmentWidget(
              ...
              StreamImageAttachment(
                ...
                onShowMessage: onShowMessage,
                //TODO: Check where to re add this ?
  ...
  /// Creates a copy of [CustomStreamMessageWidget] with
  /// specified attributes overridden.
  CustomStreamMessageWidget copyWith({
    Key? key,
  ...
  @override
  State<CustomStreamMessageWidget> createState() => _StreamMessageWidgetState();
}

class _StreamMessageWidgetState extends State<CustomStreamMessageWidget>
    with AutomaticKeepAliveClientMixin<CustomStreamMessageWidget> {
  ...
  bool get isSendFailed => widget.message.state == MessageState.sendingFailed;

  bool get isUpdateFailed =>
      widget.message.state == MessageState.updatingFailed;
  ...
  bool get hasButtons =>
      widget.message.attachments.where((it) => it.type == 'choices').isNotEmpty;
  ...
  Widget _parseButtons() {
    final attachmentGroups = <String, List<Attachment>>{};

    widget.message.attachments
        .where(
      (element) => element.type == 'choices',
    )
        .forEach((e) {
      if (attachmentGroups[e.type] == null) {
        attachmentGroups[e.type!] = [];
  ...
  void retryMessage(BuildContext context) {
    final channel = StreamChannel.of(context).channel;
    if (widget.message.state == MessageState.sendingFailed) {
      channel.sendMessage(widget.message);
      return;
    }
    if (widget.message.state == MessageState.updatingFailed) {
