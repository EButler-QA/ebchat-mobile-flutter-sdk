...
typedef MessageBuilder = Widget Function(
  ...
  MessageDetails,
  List<Message>,
  cmw.CustomMessageWidget defaultMessageWidget,
);
...
class CustomMessageListView extends StatefulWidget {
  /// Instantiate a new CustomMessageListView
...
class _MessageListViewState extends State<CustomMessageListView> {
  ...
  Widget buildParentMessage(
    ...
    final defaultMessageWidget = cmw.CustomMessageWidget(
  ...
  Widget buildMessage(Message message, List<Message> messages, int index) {
    ...
    final showTimeStamp = !widget.adminPrivilege ||
        ((!isThreadMessage || _isThreadConversation) && !hasReplies) &&
            (timeDiff >= 1 || !isNextUserSame);

    final showUsername = !isMyMessage &&
        (!widget.adminPrivilege ||
            (!isThreadMessage || _isThreadConversation) && !hasReplies) &&
        (timeDiff >= 1 || !isNextUserSame);
    ...
    final showThreadReplyIndicator =
        !_isThreadConversation && hasReplies && widget.adminPrivilege;
    final isOnlyEmoji = message.text?.isOnlyEmoji ?? false;
    ...
    Widget messageWidget = cmw.CustomMessageWidget(
      message: message,
      reverse: isMyMessage,
      ...
      showEditMessage: isMyMessage && !isMessageRead,
      showDeleteMessage: isMyMessage && !isMessageRead,
    ...
    if (widget.messageBuilder != null) {
      messageWidget = widget.messageBuilder!(
        ...
        messages,
        messageWidget as cmw.CustomMessageWidget,
      );
    }
