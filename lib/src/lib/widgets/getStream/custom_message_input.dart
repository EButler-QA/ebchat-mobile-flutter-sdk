...
/// A callback that can be passed to [CustomMessageInput.onError].
///
...
/// A callback that can be passed to [CustomMessageInput.onAttachmentLimitExceed].
///
/// This callback should not throw.
///
...
/// Location for actions on the [CustomMessageInput]
...
class CustomMessageInput extends StatefulWidget {
  ...
  /// Defines if the [CustomMessageInput] loses focuses after a message is sent.
  ...
  static MessageInputState of(BuildContext context) {
    ...
    assert(
      ...
      'You must have a CustomMessageInput widget as ancestor of your widget tree',
    );
    return messageInputState!;
  }
}

/// State of [CustomMessageInput]
class MessageInputState extends State<CustomMessageInput> {
  ...
  void _checkCommands(String s, BuildContext context) {
    if (s.startsWith('/')) {
      ...
      final command = allCommands?.firstWhereOrNull(
          (it) => it.name == 'giphy' && 'giphy' == s.substring(1));
      if (command != null) {
