...
/// Different types of hints that can be shown in [StreamCustomMessageInput].
enum HintType {
  /// Hint for [StreamCustomMessageInput] when the command is enabled and the command
  /// is 'giphy'.
  searchGif,

  /// Hint for [StreamCustomMessageInput] when there are attachments.
  addACommentOrSend,

  /// Hint for [StreamCustomMessageInput] when slow mode is enabled.
  slowModeOn,

  /// Hint for [StreamCustomMessageInput] when other conditions are not met.
  writeAMessage,
}

/// Function that returns the hint text for [StreamCustomMessageInput] based on
/// [type].
...
///               ),
///             ),
///             const StreamCustomMessageInput(),
///           ],
///         ),
///       );
/// }
...
class StreamCustomMessageInput extends StatefulWidget {
  /// Instantiate a new CustomMessageInput
  const StreamCustomMessageInput({
    super.key,
    this.onMessageSent,
  ...
  /// If true you can't send messages
  final bool disable;
  ...
  /// Defines if the [StreamCustomMessageInput] loses focuses after a message is sent.
  ...
  /// Restoration ID to save and restore the state of the CustomMessageInput.
  final String? restorationId;

  /// Wrap [StreamCustomMessageInput] with a [SafeArea widget]
  final bool? enableSafeArea;

  /// Elevation of the [StreamCustomMessageInput]
  final double? elevation;

  /// Shadow for the [StreamCustomMessageInput] widget
  ...
  @override
  StreamCustomMessageInputState createState() =>
      StreamCustomMessageInputState();
}

/// State of [StreamCustomMessageInput]
class StreamCustomMessageInputState extends State<StreamCustomMessageInput>
    with RestorationMixin<StreamCustomMessageInput>, WidgetsBindingObserver {
  ...
  @override
  void didUpdateWidget(covariant StreamCustomMessageInput oldWidget) {
  ...
  Future<void> sendMessage() async {
    ...
    if (shouldKeepFocus) {
      focusScope.requestFocus(_effectiveFocusNode);
    } else {
      focusScope.unfocus();
    }
  }
